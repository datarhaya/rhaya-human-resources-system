...
export default function LeaveHistory() {
  const { user } = useAuth();
  const isInactive = user?.employeeStatus === "INACTIVE";
  ...
  const getAvailableLeaveTypes = () => {
    const allLeaveTypes = [
      ...
      },
      {
        ...
        labelKey: "sickLeave",
        isPaid: true,
        requiresAttachment: "conditional",
        noteKey: "attachmentNote",
      },
      {
        value: "MATERNITY_LEAVE",
        labelKey: "maternityLeave",
        isPaid: true,
        requiresAttachment: false,
        femaleOnly: true,
      },
      ...
      },
      {
        ...
        labelKey: "paternityLeave",
        isPaid: true,
        requiresAttachment: false,
        maleOnly: true,
        noteKey: "paternityNote",
      },
      {
        value: "MARRIAGE_LEAVE",
        labelKey: "marriageLeave",
        isPaid: true,
        requiresAttachment: false,
      },
      {
        value: "BEREAVEMENT_LEAVE",
        labelKey: "bereavementLeave",
        isPaid: true,
        requiresAttachment: false,
        noteKey: "bereavementNote",
      },
      {
        value: "UNPAID_LEAVE",
        labelKey: "unpaidLeave",
        isPaid: false,
        requiresAttachment: false,
        noteKey: "unpaidNote",
      },
  ...
  const StatusBadge = ({ status }) => {
    ...
    return (
      <span
        className={`px-2 py-0.5 rounded text-[10px] font-black uppercase border ${styles[status]}`}
      >
        {status}
      </span>
    );
  ...
  useEffect(() => {
    if (user) {
      // If user is active and tab is history, switch to submit
      if (user.employeeStatus !== "INACTIVE" && activeTab === "history") {
        setActiveTab("submit");
      }
      // If user is inactive and tab is submit, switch to history
      if (user.employeeStatus === "INACTIVE" && activeTab === "submit") {
        setActiveTab("history");
      }
    }
  }, [user, user?.employeeStatus]); // Run when user loads

  console.log("is inactive?:", isInactive); // DEBUG
  ...
  const applyFilters = () => {
    ...
    if (filters.requestDateFrom) {
      filtered = filtered.filter(
        (req) => new Date(req.createdAt) >= new Date(filters.requestDateFrom),
      );
    }
    if (filters.requestDateTo) {
      filtered = filtered.filter(
        (req) =>
          ...
          new Date(filters.requestDateTo + "T23:59:59"),
      );
    }
    ...
    try {
      if (!formData.startDate || !formData.endDate) {
        setErrorDialog({
          ...
          messages: [
            t("leave.selectDates") || "Please select start and end dates",
          ],
          isSuccess: false,
        });
        setLoading(false);
      ...
      if (formData.leaveType !== "MATERNITY_LEAVE" && totalDays > 5) {
        setErrorDialog({
          ...
          title: t("leave.errorTitle") || "Error",
          messages: [
            "Maximum 5 working days per leave request (weekends excluded)",
          ],
          isSuccess: false,
        });
        setLoading(false);
      ...
      setErrorDialog({
        ...
        messages: [
          t("leave.submitSuccess") || "Leave request submitted successfully!",
        ],
        isSuccess: true,
      });
      ...
      setUploadProgress(0);
      const errorMessage =
        error.response?.data?.error || t("leave.submitError");
      setErrorDialog({
        show: true,
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-lg shadow-xl max-w-md w-full animate-fadeIn">
          {/* Header */}
          <div
            className={`${bgColor} text-white px-4 sm:px-6 py-3 sm:py-4 rounded-t-lg flex items-center justify-between`}
          >
              ...
              <ul className="space-y-2">
                {errorDialog.messages.map((message, index) => (
                  <li key={index} className="flex items-start">
                    <svg
                      className={`w-4 h-4 sm:w-5 sm:h-5 ${iconColor} mr-2 mt-0.5 flex-shrink-0`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
          ...
          <div className="bg-white rounded-lg shadow p-3 sm:p-4">
            ...
            <div className="text-xl sm:text-2xl font-bold text-purple-600">
              {(leaveBalance?.annualRemaining || 0) +
                (leaveBalance?.toilBalance || 0)}{" "}
              {t("leave.days")}
            </div>
            <p className="text-xs opacity-75 mt-1">
              {t("leave.annual")}: {leaveBalance?.annualRemaining || 0} |{" "}
              {t("leave.toilBalance")}: {leaveBalance?.toilBalance || 0}
            </p>
          </div>
        </div>
      )}
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-sm text-yellow-700 mt-1">
                    Your account has been deactivated. You can view your leave
                    history but cannot submit new requests.
                  </p>
                </div>
              </div>
            </div>
          ...
          {activeTab === "submit" ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <ul className="mt-1 text-xs sm:text-sm text-blue-800 list-disc list-inside space-y-1">
                      {(() => {
                        ...
                        const currentNotes = relevantNotes[
                          formData.leaveType
                        ] || ["note1"];
                        const notesToDisplay = showAllNotes
                          ? allNotes
                          : currentNotes;
                        ...
                        return (
                          <>
                            ...
                            {!showAllNotes &&
                              currentNotes.length < allNotes.length && (
              ...
              {(() => {
                ...
                const showAttachment = shouldShowAttachmentField(
                  formData.leaveType,
                );
                        ...
                        formData.endDate && (
                          ...
                          >
                            ...
                            >
                              {attachmentRequired ? (
                                <>
                                  ⚠️ Cuti sakit {totalDays} hari{" "}
                                  <strong>memerlukan</strong> surat keterangan
                                  dokter
                                </>
                              ) : (
                                <>
                                  ℹ️ Cuti sakit {totalDays} hari{" "}
                                  <strong>tidak memerlukan</strong> surat
                                  keterangan dokter (opsional)
                                </>
                              )}
                            </p>
                          </div>
                        ...
                        <div className="grid grid-cols-2 gap-y-5 gap-x-4">
                          ...
                          {request.currentApprover && (
                            <div className="space-y-0.5">
                              ...
                              <p className="text-xs font-bold text-blue-600 uppercase truncate">
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-6 py-4 space-y-4">
              ...
              {formData.attachmentFiles.length > 0 && (
                <div className="space-y-2">
                  <p className="text-sm font-medium text-gray-700">
                    {t("leave.selectedFiles") || "Selected Files"} (
                    {formData.attachmentFiles.length})
                  </p>
                  {formData.attachmentFiles.map((file, index) => (
                    <div
                      key={index}
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500">
                  {t("leave.urlExample") ||
                    "Example: Google Drive, OneDrive, or any document link"}
                </p>
              </div>
            </div>
