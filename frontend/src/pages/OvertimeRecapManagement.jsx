...
export default function OvertimeRecapManagement() {
  ...
  const LateAdditionModal = ({ isOpen, onClose, period }) => {
    ...
    const handleSubmit = async () => {
      ...
      const confirmed = confirm(
        `Re-recap ${selectedEmployees.length} employee(s) to add late-approved overtime for ${period.month}/${period.year}?\n\n` +
          `This will update their existing recap with any newly approved overtime.`,
      );
      ...
      try {
        ...
        const res = await apiClient.post("/overtime-recap/late-addition", {
        ...
        const { updated, noChanges, failed } = res.data.summary;

        alert(
          `Late Addition Complete!\n\n` +
            `✅ Updated: ${updated}\n` +
            `ℹ️ No changes: ${noChanges}\n` +
            `❌ Failed: ${failed}`,
        );
        ...
        alert(error.response?.data?.error || "Failed to add late overtime");
      } finally {
        setLoading(false);
      }
    };
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        <div className="p-6">
          <h2 className="text-xl font-bold mb-4">
            Add Late Overtime - {period.month}/{period.year}
          </h2>

          <p className="text-sm text-gray-600 mb-4">
            Select employees who have late-approved overtime that needs to be
            added to their existing recap.
          </p>
          ...
          <EmployeeMultiSelect
            ...
            filter={(emp) => emp.hasLateOvertime}
          />
          ...
          <div className="flex gap-2 mt-6">
            ...
            >
              {loading
                ? "Processing..."
                : `Update ${selectedEmployees.length} Recap(s)`}
            </button>
            <button
              onClick={onClose}
  ...
  const handleRetryFailed = async () => {
    ...
    try {
      const response = await apiClient.post("/overtime-recap/retry-failed", {
        month,
        year,
      });

      alert(
        `✅ Success: ${response.data.nowSuccess}\n❌ Still Failed: ${response.data.stillFailed}`,
      );
  ...
  const handleSendEmail = async () => {
    try {
      ...
      alert(
        `Emails sent to ${response.data.emailsSent} employees\nFailed: ${response.data.failed}`,
      );
    } catch (error) {
  ...
  const exportToExcel = () => {
    ...
    const exportData = filteredRecaps.map((r) => {
      ...
      sortedDates.forEach((dateStr) => {
        ...
        row[dateLabel] =
          totalHoursForDate > 0 ? totalHoursForDate.toFixed(1) : 0;
      });

      // Add remaining columns after date columns
      row["Division"] = r.employee.division?.name || "-";
      row["Period"] = new Date(r.year, r.month - 1).toLocaleDateString(
        "en-US",
        {
          year: "numeric",
          month: "long",
      ...
      <div className="mb-6 grid grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-lg shadow p-4">
          ...
          <p className="text-2xl font-bold text-blue-600">
            {filteredRecaps.reduce((sum, r) => sum + r.paidHours, 0).toFixed(1)}
            h
          </p>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
                ...
                return (
                  <React.Fragment key={r.id}>
                    ...
                    <tr
                      ...
                      onClick={() => toggleRecapExpansion(r.id)}
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {new Date(r.year, r.month - 1).toLocaleDateString(
                          "en-US",
                          {
                            year: "numeric",
                            month: "long",
                      ...
                      <td className="px-6 py-4 text-right text-orange-600">
                        {r.excessHours > 0
                          ? `${r.excessHours.toFixed(1)}h`
                          : "-"}
                      </td>
                      <td className="px-6 py-4 text-right">
                        {r.toilDaysCreated > 0 ? (
                          <span className="px-2 py-1 bg-green-100 text-green-800 rounded-full text-sm font-medium">
                            {r.toilDaysCreated} day
                            {r.toilDaysCreated > 1 ? "s" : ""}
                          </span>
                        ) : (
                          "-"
                        )}
                              ...
                              <div className="bg-white rounded-lg p-4 shadow-sm">
                                ...
                                <dl className="space-y-2">
                                  ...
                                  <div className="flex justify-between text-sm border-t pt-2">
                                    ...
                                    <dd className="font-medium text-gray-900">
                                      {r.carryoverHours.toFixed(1)} hrs
                                    </dd>
                                  </div>
                                  <div className="flex justify-between text-sm">
                                    <dt className="text-gray-600">
                                    ...
                                    <dd className="font-medium text-purple-600">
                                      {r.totalToilHours.toFixed(1)} hrs
                                    </dd>
                                  </div>
                                  <div className="flex justify-between text-sm">
                                    <dt className="text-gray-600">
                                  ...
                                  <div className="flex justify-between text-sm">
                                    ...
                                    <dd className="font-medium text-gray-900">
                                      {r.remainingHours.toFixed(1)} hrs
                                    </dd>
                                  </div>
                                  <div className="flex justify-between text-sm border-t pt-2">
                                    <dt className="text-gray-600">
                                    ...
                                    <dd className="font-medium text-gray-900">
                                      Rp{" "}
                                      {(
                                        (r.employee.overtimeRate || 300000) / 8
                              ...
                              <div className="bg-white rounded-lg p-4 shadow-sm">
                                ...
                                <dl className="space-y-2">
                                  ...
                                  <div className="text-sm">
                                    ...
                                    <dd className="font-medium text-gray-900 mt-1">
                                      {r.overtimeRequests?.length || 0}{" "}
                                      request(s)
                                    </dd>
                                  </div>
                                  <div className="text-sm">
                                    <dt className="text-gray-600">
                                      Number of Overtime Dates:
                                    </dt>
                                    <dd className="font-medium text-gray-900 mt-1">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setRecapConfig({ ...recapConfig, fromDate: e.target.value })
                  }
                  className="w-full px-3 py-2 border rounded-lg"
                  required
                />
                <p className="text-xs text-gray-500 mt-1">
                  Start date of overtime period
                </p>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setRecapConfig({ ...recapConfig, toDate: e.target.value })
                  }
                  className="w-full px-3 py-2 border rounded-lg"
                  required
                />
                <p className="text-xs text-gray-500 mt-1">
                  Last day included in this recap
                </p>
              </div>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <p className="text-sm text-blue-800">
                  <strong>Period:</strong> {recapConfig.fromDate || "(not set)"}{" "}
                  to {recapConfig.toDate}
                  <br />
                  <strong>Payroll:</strong> {monthNames[recapConfig.month - 1]}{" "}
                  {recapConfig.year}
                </p>
              </div>
            ...
            <div className="flex justify-between items-start mb-4">
              <div>
                <h2 className="text-xl font-bold text-red-600">
                  ⚠️ Cannot Start Recap
                </h2>
                <p className="text-sm text-gray-600 mt-1">
                  Resolve previous failures first
                </p>
              </div>
              <button
            ...
            <div className="space-y-4">
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                <p className="text-sm text-red-800">
                  You have{" "}
                  <strong>
                    {blockingFailures.reduce((sum, f) => sum + f.count, 0)}{" "}
                    failed recap(s)
                  </strong>{" "}
                  from previous months that must be resolved before starting a
                  new recap.
                </p>
              </div>
              ...
              {blockingFailures.map((failure, idx) => (
                <div key={idx} className="border rounded-lg p-4">
                  <h3 className="font-semibold text-gray-900 mb-3">
                    {monthNames[failure.month - 1]} {failure.year} -{" "}
              ...
              <div className="flex space-x-3 pt-4">
                ...
                >
                  Go to {monthNames[blockingFailures[0]?.month - 1]}{" "}
                  {blockingFailures[0]?.year}
                </button>
              </div>
            </div>
          </div>
            ...
            <div className="flex justify-between items-start mb-4">
              <div>
                <h2 className="text-xl font-bold">
                  Recap Complete - {monthNames[recapConfig.month - 1]}{" "}
                  {recapConfig.year}
                </h2>
                <p className="text-sm text-gray-600 mt-1">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                ...
                <p className="text-3xl font-bold text-green-600">
                  {recapResults.success?.length || 0}
                </p>
              </div>
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                <p className="text-sm text-gray-600">Failed</p>
                <p className="text-3xl font-bold text-red-600">
                  {recapResults.failed?.length || 0}
                </p>
              </div>
            </div>
            ...
            {recapResults.success && recapResults.success.length > 0 && (
              <div className="mb-4">
                <h3 className="font-semibold mb-3 text-green-600">
                  ✅ Successfully Processed ({recapResults.success.length})
                </h3>
                <div className="bg-gray-50 rounded-lg p-3 text-sm text-gray-600">
                  All employees have been recapped successfully. Check the main
                  table for details.
                </div>
              </div>
            )}
            ...
            <div className="space-y-4">
              <div className="bg-gray-50 border rounded-lg p-3">
                <label className="text-sm font-medium text-gray-600">
                  Current Last Recap Date:
                </label>
                <p className="text-lg font-bold text-gray-900 mt-1">
                  {systemSettings?.lastRecapDate
                    ? format(
                        parseISO(systemSettings.lastRecapDate),
                        "dd MMMM yyyy",
                      )
                    : "Not set"}
                </p>
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-blue-800">
                      ...
                      <strong>eligible employees only</strong>
                    </p>
                        ...
                        users.filter(
                          (u) =>
                            ![
                              ...
                              "ADMIN",
                              "FREELANCE",
                            ].includes(u.employeeStatus),
                        ).length
                      }{" "}
                      employees
                    </p>
                    <p className="text-xs text-gray-500 mt-2">
                      <strong>Excluded:</strong> Resigned, Inactive, Admin,
                      Freelance
                    </p>
                  </div>
                </div>
              ...
              <div className="border rounded-lg p-4 space-y-3">
                <div>
                  ...
                  <p className="text-sm font-medium mt-1">
                    [OVERTIME REMINDER] Batas Akhir Submit Lembur -{" "}
                    {recapConfig.toDate
                      ? format(parseISO(recapConfig.toDate), "dd MMMM yyyy")
                      : "(date not set)"}
                  </p>
                </div>
                ...
                <div className="border-t pt-3">
                  ...
                  <div className="mt-2 bg-gray-50 border rounded-lg p-4 text-sm space-y-2">
                    <p>
                      <strong>Kepada [Nama Karyawan],</strong>
                    </p>
                    <p>Dengan hormat,</p>
                    <p>
                      ...
                      , adalah <strong>batas akhir</strong> untuk submit lembur
                      periode payroll bulan ini.
                    </p>
                    ...
                    <div className="bg-white border rounded p-3 my-3">
                      ...
                      <p className="text-xs space-y-1">
                        <span className="block">
                          Periode:{" "}
                          <strong>
                            {monthNames[recapConfig.month - 1]}{" "}
                            {recapConfig.year}
                          </strong>
                        </span>
                        <span className="block">
                          Dari:{" "}
                          <strong>
                            {recapConfig.fromDate
                              ? format(
                                  parseISO(recapConfig.fromDate),
                                  "dd MMMM yyyy",
                                )
                              : "(not set)"}
                          </strong>
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 rounded p-3">
                      ...
                      <p className="text-xs text-yellow-800 space-y-1">
                        ✓ Semua lembur HARUS sudah disubmit hari ini
                        <br />
                        ✓ Setelah hari ini, submit akan DIKUNCI
                        <br />
                        ✓ Pastikan sudah diapprove atasan
                        <br />✓ Lembur yang belum diapprove tidak akan masuk
                        payroll
                      </p>
                    </div>
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                <p className="text-xs text-yellow-800">
                  ...
                  <br />• This email will be sent immediately to all active
                  employees
          ...
          <div className="bg-white rounded-lg max-w-4xl w-full p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">
                  {monthNames[recapConfig.month - 1]} {recapConfig.year} -{" "}
                  {failedCount} employees failed
                </p>
              </div>
              <button
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-start justify-between">
                  <div>
                    <p className="font-medium text-red-900">
                      Recap Failed for {failedCount} Employees
                    </p>
                    <p className="text-sm text-red-700 mt-1">
                      These employees have pending overtime requests that need
                      approval before they can be recapped.
                    </p>
                  </div>
                  <button
                    onClick={() => {
                        ...
                        <div className="flex-1">
                          <div className="flex items-center space-x-3 mb-2">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-gray-600">
                                ...
                                {recap.employee.division?.name || "No Division"}
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="ml-11 mt-3 flex items-center space-x-2 text-xs">
                            <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded">
                              💡 Action needed: Get supervisor to approve
                              pending overtimes
                            </span>
                          </div>
                        </div>
              ...
              {filteredRecaps.filter((r) => r.recapStatus === "failed")
                .length === 0 && (
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p className="text-sm">
                    All employees have been recapped successfully
                  </p>
                </div>
              )}
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <ol className="text-sm text-blue-800 space-y-1 list-decimal list-inside">
                  ...
                  <li>
                    Ask them to approve or reject the pending overtime requests
                  </li>
                  <li>
                    Once approved/rejected, click "Retry All Failed" or retry
                    individual employees
                  </li>
                  <li>
                    System will automatically process them if there are no more
                    pending requests
                  </li>
                </ol>
              </div>
                ...
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-green-800">
                      This will process overtime recap for{" "}
                      <strong>ALL employees</strong> who have approved overtime
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                <div className="flex items-start">
                  ...
                  <p className="text-xs text-yellow-800">
                    ...
                    sure you've selected the correct period.
                  </p>
                </div>
              </div>
